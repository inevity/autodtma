...
conn_init (Conn *conn)
{
  ...
  if(param.des_ports.num_ports > 0)
  {
	  /* determine server address to use */
	  conn->port = param.des_ports.des_port[param.des_ports.next_port];
	  param.des_ports.next_port =(param.des_ports.next_port+1) % param.des_ports.num_ports;
  }
  else
  {
