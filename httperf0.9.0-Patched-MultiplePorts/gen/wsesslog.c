		...
		dollorparse=stristr(nuri,"$parse");
			if(dollorparse)
			{
				...
				if(current!=NULL)
				{
					...
					if(newuri!=NULL)
					{
						free(priv->current_burst->next->req_list->uri);
						priv->current_burst->next->req_list->uri=newuri;
						priv->current_burst->next->req_list->uri_len=strlen(newuri);
						freelink(readcurrent);
					}
					else
					{
						free(priv->current_burst->next->req_list->uri);
						priv->current_burst->next->req_list->uri=oldnuri;
						priv->current_burst->next->req_list->uri_len=strlen(oldnuri);
					}
					freelink(current);			
				}
