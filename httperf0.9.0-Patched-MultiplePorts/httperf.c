...
static struct option longopts[] =
  {
    ...
    {"des-ports",     required_argument, (int *) &param.des_ports,       0},
	...
	case 0:
	  ...
	  if (flag == &param.method)
	      ...
	      while(numSrcs < MAX_SRC_ADDRS)
		{
		  ...
		  /* convert the address */
		  resultS = inet_aton(optarg, &src_addr);
		  if(resultS == 0)
		    {
...
#if 1
	      for(numSrcs=0; numSrcs<param.src_addrs.num_addrs; numSrcs++)
		fprintf(stdout," %d %d  %x\n",numSrcs,param.src_addrs.src_addr[numSrcs].s_addr, 
			ntohl(param.src_addrs.src_addr[numSrcs].s_addr));
#endif
	    }
#endif /* MULTIPLE_SRC_ADDRS_OPTION */
#ifdef MULTIPLE_PORT_OPTION
	  else if (flag == &param.des_ports)
	    {
	      errno = 0;
	      numPort=0;
		 
		  param.des_ports.num_ports=0;
	     fprintf(stdout,"%s",optarg);
		  while(numPort < MAX_DES_PORTS)
		{
		  /* determine address */
		  if((end = strchr(optarg, ',')) == NULL)
		    {
		      /* last address */
		      numPort = param.des_ports.num_ports;
			  printf("number of ports =%d",numPort);
		    }
		  else
		    {
		  ...
		  /* store the address */
	      errno = 0;
		  param.des_ports.des_port[param.des_ports.num_ports++]= strtoul (optarg, NULL, 10);
		  if (errno == ERANGE 
		  || (unsigned) param.des_ports.des_port[param.des_ports.num_ports-1] > 0xffff)
		{
		  fprintf (stderr, "\n%s: illegal port list  %d\n",
			   prog_name, param.des_ports.des_port[param.des_ports.num_ports-1]);
		  exit (1);
		}
		  //fprintf(stdout,"the %d port is %d",param.des_ports.num_ports-1,param.des_ports.des_port[param.des_ports.num_ports-1]);
		  /* advance to next address */
		  if (! end)
			  break;
		  optarg = end;
...
/*\RSH*/
  ...
  printf("\n*** This version supports multiple destination ports in a single run ***\n\n");
